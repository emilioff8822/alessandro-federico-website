...
function PlaceholderSlot({ label }: { label: string }) {
  return (
    <div className="w-full aspect-square flex flex-col items-center justify-center gap-3 bg-azzurro-chiaro">
          ...
          {filtri.map((f) => {
            ...
            return (
              <button
                ...
                aria-pressed={attivo}
                className={`px-5 py-2.5 rounded-full border eyebrow text-[11px] tracking-[0.14em] transition-all duration-300 ${
                  attivo
                    ? "bg-blu-scuro border-blu-scuro text-white"
                    : "bg-transparent border-brand-blu text-blu-notte hover:bg-azzurro-chiaro"
                }`}
              >
                {f.label}
              </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {casiFiltrati.map((caso, index) => (
          <FadeIn key={caso.id} delay={index * 0.06}>
            <article className="h-full rounded-2xl overflow-hidden border border-brand-blu/40 bg-white flex flex-col shadow-[0_4px_28px_rgba(30,53,80,0.06)]">
              <div className="grid grid-cols-2 gap-px bg-brand-blu/40">
